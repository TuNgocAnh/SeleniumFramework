...
public class LoginDataDrivenTests extends BaseTest {
    ...
    @Test(groups = {"regression"}, dataProvider = "loginData", description = "Data-driven login từ JSON")
    public void loginWithData(Map<String, Object> row) {
        ...
        if (expectSuccess) {
            Assert.assertTrue(new ProductsPage().isLoaded(), "Login đúng nhưng không vào Products: " + user);
        } else {
