...
public class LoginTests extends BaseTest {
    ...
    @Test(groups = {"smoke", "regression"}, description = "Login thành công với standard_user")
    ...
    @Test(groups = {"regression"}, description = "Login với password sai → hiển thị error")
    public void loginWrongPassword() {
        ...
        Assertions.soft().assertTrue(
                login.getErrorMessage().contains("Username and password do not match"),
                "Nội dung error không khớp");
        Assertions.assertAll();
    }

    @Test(groups = {"regression"}, description = "Locked user không login được")
    public void loginLockedUser() {
        LoginPage login = new LoginPage().login(CredentialsManager.user("locked"), CredentialsManager.pass("locked"));
