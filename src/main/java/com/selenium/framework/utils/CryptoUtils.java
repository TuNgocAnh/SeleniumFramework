...
/**
 * AES-256/GCM. Khoá lấy từ env CRED_KEY (hoặc system property cred.key).
 * Nếu không có khoá, fallback sang Base64 (chỉ obfuscation — không bảo mật thật).
 * Định dạng chuỗi đã mã hoá: "enc:<base64(iv||ciphertext)>" hoặc "b64:<base64>".
 */
public final class CryptoUtils {
    ...
    public static String encrypt(String plain) {
        byte[] key = readKey();
        if (key == null) return B64_PREFIX + Base64.getEncoder().encodeToString(plain.getBytes(StandardCharsets.UTF_8));
        try {
    ...
    public static String decrypt(String value) {
        ...
        if (value.startsWith(B64_PREFIX)) {
            return new String(Base64.getDecoder().decode(value.substring(B64_PREFIX.length())), StandardCharsets.UTF_8);
        }
