...
public final class DriverFactory {
    ...
    public static void initDriver(String browser) {
        ...
        if (!gridUrl.isBlank()) {
            ...
            if (browser.equalsIgnoreCase("chrome"))  setupWdm(WebDriverManager.chromedriver(), "chromeVersion");
            if (browser.equalsIgnoreCase("edge"))    setupWdm(WebDriverManager.edgedriver(),   "edgeVersion");
            if (browser.equalsIgnoreCase("firefox")) setupWdm(WebDriverManager.firefoxdriver(),"firefoxVersion");
        }

        if (!mobile) driver.manage().window().maximize();
        driver.manage().timeouts().implicitlyWait(Duration.ZERO);
        driver.manage().timeouts().pageLoadTimeout(
                Duration.ofSeconds(ConfigReader.getInt("pageLoadWait", 30)));
        driver.manage().timeouts().scriptTimeout(
                Duration.ofSeconds(ConfigReader.getInt("scriptTimeout", 30)));
        DRIVER.set(driver);
    }
    ...
    private static ChromeOptions chromeOptions(boolean headless, boolean mobile) {
        ...
        if (headless) opts.addArguments("--headless=new", "--window-size=1920,1080", "--no-sandbox", "--disable-dev-shm-usage");
        if (mobile) {
    ...
    private static WebDriver createRemote(String browser, boolean headless, boolean mobile, String gridUrl) {
